...
namespace 商城百媚.Controllers
{
    ...
    public class ShopController : Controller
    {
        ...
        public ActionResult addCart(int id, int num = 1)
        {
            ...
            {
                return Content("<script>alert('库存不足');window.location.href = '/Car/Details/" + id + "';</script>");
            }
            ...
            if (Session["cart"] != null)
            {
                ...
                {
                    //商品在购物车中不存在，new一个新的cart，添加属性值并添加到carts集合
                    cart = new ShopCart();
                    cart.Number = num;
        ...
        public ActionResult Add(int id)
        {
            if (Session["cart"] != null)
            {
                ...
                {
                    return Content("<script>alert('库存不足');window.location.href = '/Shop/Index;</script>");
                }
